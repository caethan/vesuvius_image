"""
Fast, GPU-enabled 3D non-local means implementation.

Built off of the skimage implementation, with manual tuning and custom kernels
for faster handling of 3D regions.
"""
...
if DEVICE == "gpu":
    """N.B. these kernels will be much slower the first time they are run because the kernel will need
    to be compiled.  Only do timing post-compilation.
    """
    ...
    def fast_approx_exp(y):
        y = xp.ascontiguousarray(y, dtype=xp.float64)
        y.view(xp.int32)[..., EXPSLICE] = (EXP_A * y + EXP_BC).astype(xp.int32)
...
def fast_nl_means_denoising_3d(image, s=5, d=7, h=0.1, var=0.0):
    if s % 2 == 0:
    ...
    padded = xp.ascontiguousarray(
        xp.pad(image, ((pad_size, pad_size), (pad_size, pad_size), (pad_size, pad_size)), mode='reflect'),
        dtype=xp.float64,
    )
    
    summed_area_table = xp.zeros(
        ((image.shape[0] + 2 * offset), (image.shape[1] + 2 * offset), (image.shape[2] + 2 * offset)),
        dtype=xp.float64,
    )
    
    weights = xp.zeros(padded.shape, dtype=xp.float64)
    result = xp.zeros(padded.shape, dtype=xp.float64)
    ...
    for t_pln, t_row, t_col in iter_directions(d):
        ...
        xp.square(summed_area_table, out=summed_area_table)
        xp.subtract(summed_area_table, var, out=summed_area_table)
        xp.cumsum(summed_area_table, axis=0, out=summed_area_table)
        xp.cumsum(summed_area_table, axis=1, out=summed_area_table)
        xp.cumsum(summed_area_table, axis=2, out=summed_area_table) 
        ...
        xp.clip(local_weight, None, 0, out=local_weight)
        ...
        result[pslice_offset, rslice_offset, cslice_offset] += xp.multiply(local_weight, padded[slice_simple, slice_simple, slice_simple])

        weights[pslice_offset, rslice_offset, cslice_offset] += local_weight
        result[slice_simple, slice_simple, slice_simple] += xp.multiply(local_weight, padded[pslice_offset, rslice_offset, cslice_offset])

    # Normalize pixel values using sum of weights of contributing patches and return without padding
    return result[slice_simple, slice_simple, slice_simple] / weights[slice_simple, slice_simple, slice_simple]
...
def nlm_3d(image, sigma, patch_size=7, patch_distance=1, h=0.03):                    
    ...
    if DEVICE == "gpu":
        ...
        return fast_nl_means_denoising_3d(image, s=patch_size, d=patch_distance, h=h, var=2 * sigma * sigma)